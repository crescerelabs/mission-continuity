...
REPLAY_LABEL = ("AI-generated visual reconstruction of recorded execution. The footage is generated by FLUX 3 "
                "(Black Forest Labs) and is illustrative only: it is not evidence. Every caption, number and time "
                "is taken from the recorded run, and each scene links to its source records below.")
...
def render_replay():
    ...
    if not info:
        ...
        return
    st.warning(REPLAY_LABEL, icon="🎞️")
    if info["placeholder"]:
        st.error("Local preview: placeholder backgrounds, no generated footage.")
    rj, spec = info["render"], info["scenes"]
    st.video((info["dir"] / rj["video"]).read_bytes())
    v = _replay_verified(label, (info["dir"] / "render.json").stat().st_mtime, info["placeholder"])
    sources = {t["scene_id"]: t["footage"] for t in rj["timeline"]}
    st.caption(f"{rj['duration_s']} s · not to scale (the recorded run took {src.j('results.json', {}).get('cost', {}).get('wall_seconds', '?')} s) · "
               f"footage per scene: " + ", ".join(f"{k} {v_}" for k, v_ in sources.items()) +
               f" · verification: **{'PASS' if v['ok'] else 'FAIL'}** (scenes rebuilt from the records, source rows, "
               f"video and clip hashes) · video SHA-256 {rj['video_sha256'][:12]}…")
    st.markdown(f"[Open this run's Investigation timeline](?screen=investigation&bundle={label})")
    st.subheader("Scenes and their source evidence")
    tl = {t["scene_id"]: t for t in rj["timeline"]}
    for sc in spec["scenes"]:
        t = tl.get(sc["scene_id"], {})
        rec = f" · recorded {sc['offset']} ({sc['recorded_ts']})" if sc.get("recorded_ts") else ""
        st.markdown(f"**{sc['scene_id']} · video {t.get('start_s', '?')}–{t.get('end_s', '?')} s{rec}** · "
                    f"footage: {t.get('footage')}  \nOn video: _{md(sc['caption'])}_  \n{md(sc['detail'])}")
        refs = []
        for r in sc["sources"]:
            ref = f"`{r['file']}" + (f":{r['line']}" if r.get("line") else "") + "`"
            ref += f" {r['json_pointer']}" if r.get("json_pointer") else ""
            ids = r.get("event_id") or r.get("tool_call_id") or r.get("compaction_id")
            ref += f" · {r.get('event_type', '')} `{ids}`" if ids else ""
            ref += f" · row SHA-256 `{r['row_sha256'][:16]}…`" if r.get("row_sha256") else ""
            refs.append(ref)
        st.caption("Source records: " + " · ".join(refs))
    with st.expander("Provenance (generation requests, BFL task ids, cost, hashes)"):
        for sc in spec["scenes"]:
            if sc.get("prompt"):
                f = info["footage"].get(sc["scene_id"], {})
                st.markdown(f"**{sc['scene_id']}** · footage: {sources.get(sc['scene_id'])}"
                            + (f" · FLUX 3 task `{f.get('task_id')}` · {f.get('status')} · {f.get('latency_s')} s · "
                               f"reported cost {f.get('cost_usd_reported')} USD · clip SHA-256 {str(f.get('clip_sha256'))[:12]}…"
                               f" · {'accepted' if f.get('accepted') else 'not accepted'}" if f else ""))
                st.caption("Prompt: " + sc["prompt"])
        st.json({"render": rj, "verification": v["checks"]}, expanded=False)
...
def render_comparison():
    ...
    if not rows:
        ...
        if mode == "Replay":
            ...
            st.caption("AI-generated visual reconstructions of recorded runs; footage illustrative, captions from the record.")
            cols = st.columns(2)
